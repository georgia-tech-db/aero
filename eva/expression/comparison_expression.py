...
class ComparisonExpression(AbstractExpression):
    ...
    def get_uncached_ratio(self, batch: Batch, **kwargs) -> float:
        if isinstance(self.get_child(0), FunctionExpression):
            return self.get_child(0).get_uncached_ratio(batch)
        if isinstance(self.get_child(1), FunctionExpression):
            return self.get_child(1).get_uncached_ratio(batch)
        return 1
