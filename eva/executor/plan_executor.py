...
from eva.experimental.eddy.executor.eddy_executor import EddyExecutor, LaminarExecutor
...
class PlanExecutor:
    ...
    def _build_execution_tree(self, plan: AbstractPlan) -> AbstractExecutor:
        ...
        elif plan_opr_type == PlanOprType.EDDY:
            # Eddy needs to build inner plan.
            executor_node = EddyExecutor(node=plan)
            inner_executor_list = []
            for inner_plan in executor_node.inner_plan_list:
                inner_executor_list.append(self._build_execution_tree(inner_plan))
            executor_node.build_inner_executor_list(inner_executor_list)
            executor_node.populate_input_queue_list()
            for inner_executor in executor_node.inner_executor_list:
                inner_executor.populate_input_queue_list()
        elif plan_opr_type == PlanOprType.LAMINAR:
            # Laminar needs to build inner plan.
            executor_node = LaminarExecutor(node=plan)
    ...
    def _get_execution_tree_str(self, executor: AbstractExecutor, depth: int = 0):
        execution_tree_str = (
            depth * 4 * " " + "|__" + str(executor.__class__.__name__) + "\n"
        )
        for child in executor.children:
            execution_tree_str += self._get_execution_tree_str(child, depth + 1)
        return execution_tree_str
    ...
    def execute_plan(self) -> Iterator[Batch]:
        ...
        try:
            ...
            execution_tree_str = self._get_execution_tree_str(execution_tree)
            logger.info(f"\nExecution tree\n{execution_tree_str}")
