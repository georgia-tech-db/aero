...
class PlanGenerator:
    ...
    def build(self, logical_plan: Operator):
        logger.info(f"\nLogical Plan\n{self._get_logical_plan_str(logical_plan, 0)}")
    ...
    def _get_logical_plan_str(self, logical_plan, depth=0):
        cur_str = " " * depth * 4 + "|__ " + str(logical_plan.__class__.__name__) + "\n"
        for child in logical_plan.children:
            cur_str += self._get_logical_plan_str(child, depth + 1)
        return cur_str
