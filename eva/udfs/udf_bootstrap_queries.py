...
Area_udf_query = """CREATE UDF IF NOT EXISTS Area
            INPUT  (bboxes NDARRAY FLOAT32(ANYDIM, 4))
            OUTPUT (area INTEGER)
            TYPE  NdarrayUDF
            IMPL  "{}/udfs/{}/area.py";
        """.format(
...
Color_udf_query = """CREATE UDF IF NOT EXISTS Color
                INPUT (data NDARRAY UINT8(3, ANYDIM, ANYDIM))
                OUTPUT (color TEXT(1000))
                TYPE NdarrayUDF
                IMPL "{}/udfs/{}/color.py";
        """.format(
    EVA_INSTALLATION_DIR, NDARRAY_DIR
...
Dog_breed_udf_query = """CREATE UDF IF NOT EXISTS DogBreedClassifier
        INPUT  (frame NDARRAY UINT8(3, ANYDIM, ANYDIM))
        OUTPUT (labels NDARRAY STR(10))
        TYPE  Classification
        IMPL  '{}/udfs/dog_breed_classifier.py';
        """.format(
    EVA_INSTALLATION_DIR
)

Scene_udf_query = """CREATE UDF IF NOT EXISTS SceneClassifier
        INPUT  (frame NDARRAY UINT8(3, ANYDIM, ANYDIM))
        OUTPUT (labels NDARRAY STR(10))
        TYPE  Classification
        IMPL  '{}/udfs/scene_classifier.py';
        """.format(
    EVA_INSTALLATION_DIR
)

Hard_hat_udf_query = """CREATE UDF IF NOT EXISTS HardHatDetector 
                  INPUT  (frame NDARRAY UINT8(3, ANYDIM, ANYDIM))
                  OUTPUT (labels NDARRAY STR(ANYDIM), bboxes NDARRAY FLOAT32(ANYDIM, 4),
                          scores NDARRAY FLOAT32(ANYDIM))
                  TYPE  Classification
                  IMPL  '{}/udfs/hard_hat_detector.py';
        """.format(
    EVA_INSTALLATION_DIR
)

LLM_udf_query = """CREATE UDF IF NOT EXISTS LLM 
                  INPUT  (text NDARRAY STR(ANYDIM))
                  OUTPUT (response NDARRAY STR(ANYDIM))
                  TYPE  Classification
                  IMPL  '{}/udfs/llm.py';
        """.format(
    EVA_INSTALLATION_DIR
