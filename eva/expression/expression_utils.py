...
def get_expr_tree_str(expr, depth=0):
    if isinstance(expr, ConstantValueExpression):
        cur_expr_str = (
            " " * 4 * depth
            + "|__ "
            + str(expr.__class__.__name__)
            + f"({expr.value})"
            + "\n"
        )
    elif isinstance(expr, ComparisonExpression) or isinstance(expr, LogicalExpression):
        cur_expr_str = (
            " " * 4 * depth
            + "|__ "
            + str(expr.__class__.__name__)
        ...
        cur_expr_str = (
            ...
            + "|__ "
            + str(expr.__class__.__name__)
            + f"({expr.table_alias}.{expr.col_name})"
            + "\n"
        )
    else:
        cur_expr_str = " " * 4 * depth + "|__ " + str(expr.__class__.__name__) + "\n"

    for child_expr in expr.children:
        cur_expr_str += get_expr_tree_str(child_expr, depth + 1)
    return cur_expr_str
